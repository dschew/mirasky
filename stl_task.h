 ...
 *
 *  License
 *    This file released under the Lesser GNU Public License. This program is for 
 ...
 *    \li running - The task's run() function is executing
 *    \li pending - The task needs to run again as soon as possible
 *    \li waiting - The task is waiting for its next run time to occur (starting state)
 *    \li blocked - The task is prevented from running because a resource is unavailable
 *    \li suspended - The task is turned off until somebody calls its resume() method
 *  These states are not the same as the states which are programmed by the user; these
 *  states are only used by the task scheduler to figure out which task to run at a 
 *  given time according to priorities (if used) and which tasks are asking to be run. 
 */

typedef enum task_op_state {TASK_RUNNING, TASK_PENDING, TASK_WAITING, TASK_BLOCKED,
    TASK_SUSPENDED};
